...
height_object <- c(height)
height_object

weight_object <- c(weight)
weight_object
...
new_women_df <- data.frame(weight_object, height_object)
new_women_df
...
height_greater_than_65 = subset(new_women_df, height_object>65)
height_greater_than_65

subset(height_object, height_object>65)
...
women_df_scatter <- ggplot(new_women_df, aes(x=height_object, y=weight_object)) + 
  geom_point()

women_df_scatter

#Optional 'Just for fun' TASK: Change the arguments for the geom point and see what happens to the plot
women_df2_scatter <- ggplot(new_women_df, aes(x=height_object, y=weight_object)) +
  geom_point(shape = 21, colour = "grey", fill = "yellow", size = 2, stroke = 2)
